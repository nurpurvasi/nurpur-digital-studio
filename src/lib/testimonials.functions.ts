...
const SELECT_COLS =
  "id, client_name, company, designation, testimonial, rating, client_photo, company_logo, location, featured, sort_order, status, publish_date, seo_title, seo_description, created_at, updated_at";
...
export const listPublicTestimonials = createServerFn({ method: "GET" }).handler(async () => {
  ...
  if (error) return { testimonials: [] as Testimonial[] };
...
export const getAdminTestimonial = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await (context.supabase as any)
      ...
      .maybeSingle();
    if (error) throw error;
    return { testimonial: (row ?? null) as Testimonial | null };
  });
...
const testimonialInput = z.object({
  ...
  client_photo: z.string().max(1000).default(""),
  company_logo: z.string().max(1000).default(""),
...
export const upsertTestimonial = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await (context.supabase as any)
      ...
      .single();
    if (error) throw error;
    return { testimonial: row as Testimonial };
  });
...
export const duplicateTestimonial = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: { id: string }) => z.object({ id: z.string().uuid() }).parse(i))
  .handler(async ({ data, context }) => {
    await assertAdmin(context);
    ...
    const copy = {
      ...row,
      id: undefined,
      client_name: row.client_name ? `${row.client_name} (Copy)` : "(Copy)",
      status: "draft",
      publish_date: null,
      featured: false,
      created_by: context.userId,
    ...
    return { testimonial: inserted as Testimonial };
  });
