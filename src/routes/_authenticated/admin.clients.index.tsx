...
function AdminClientsList() {
  ...
  const load = useServerFn(listAdminClients);
  const save = useServerFn(upsertClient);
  const del = useServerFn(deleteClient);
  const dup = useServerFn(duplicateClient);
  const reorder = useServerFn(reorderClients);
  ...
  const [items, setItems] = useState<ClientBrand[]>([]);
  ...
  const invalidate = () => {
    ...
    qc.invalidateQueries({ queryKey: ["clients-public"] });
  };

  const publishMut = useMutation({
    mutationFn: (c: ClientBrand) => save({ data: { ...c, published: !c.published } }),
    onSuccess: invalidate,
  });
  const featureMut = useMutation({
    mutationFn: (c: ClientBrand) => save({ data: { ...c, featured: !c.featured } }),
    onSuccess: invalidate,
  });
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fc]">
        ...
        <div className="mx-auto flex h-16 max-w-[1400px] items-center gap-3 px-4 sm:px-6">
          ...
          <div className="ml-2 flex items-center gap-2">
            ...
            <div className="leading-tight">
              <div className="text-sm font-semibold tracking-tight">Clients & Brands</div>
          ...
          <div className="ml-auto">
            <button
              onClick={() => navigate({ to: "/admin/clients/$id", params: { id: "new" } })}
      ...
      <main className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6">
        {isEmpty ? (
          <EmptyState
            onCreate={() => navigate({ to: "/admin/clients/$id", params: { id: "new" } })}
          />
        ) : (
          <>
              ...
              <>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {paged.map((c) => (
                    ...
                    >
                      <div className="relative grid h-28 place-items-center border-b border-border/60 bg-[#fbfcff] p-4">
                        <GripVertical className="absolute left-2 top-2 h-3.5 w-3.5 cursor-grab text-muted-foreground opacity-0 transition group-hover:opacity-100" />
                        {c.logo ? (
                          <img
                            src={c.logo}
                      ...
                      <div className="flex flex-1 flex-col gap-2 p-4">
                        <div className="truncate text-sm font-semibold tracking-tight">
                          {c.company_name || "Untitled client"}
                        </div>
    ...
    <div className="relative overflow-hidden rounded-3xl border border-border bg-white p-10 text-center sm:p-16">
      ...
      <div className="relative mx-auto max-w-md">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Upload a logo, add a website and category. Featured clients appear in the homepage “Trusted
          by” carousel automatically.
        </p>
